...
exports.startGame = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error starting game", details: error.message });
  }
};
...
exports.makeMove = async (req, res) => {
  ...
  if (!number) {
    return res
      .status(400)
      .json({ error: "A number is required to make a move" });
  }
  ...
  if (isNaN(selectedNumber) || selectedNumber < 0 || selectedNumber > 9) {
    return res
      .status(400)
      .json({ error: "Please enter a valid number between 0 and 9." });
  }

  try {
    console.log("Making move with number:", selectedNumber);
    const response = await axios.get(
      `http://marcconrad.com/uob/banana/api.php?num=${selectedNumber}`
    );
    ...
    res
      .status(500)
      .json({ error: "Error making move", details: error.message });
  }
};
...
exports.getResult = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error getting result", details: error.message });
  }
};
